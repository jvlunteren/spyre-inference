...
class TestVariants:
    ...
    def test_batched_variants_cover_both_flag_states(self, bucketer):
        ...
        for (num_seqs, _), flags in by_size.items():
            ...
            assert ((True, False) in flags) == (num_seqs > _MIN_BATCHED_SEQS)
    ...
    def test_batched_variants_skip_unreachable_seqs_buckets(self, bucketer):
        """build() only takes the batched path at or above _MIN_BATCHED_SEQS."""
        from spyre_inference.v1.attention.backends.spyre_attn import _MIN_BATCHED_SEQS

        assert {v.num_seqs for v in bucketer.batched_variants()} == {
            n for n in bucketer.num_seqs_buckets if n >= _MIN_BATCHED_SEQS
        }
